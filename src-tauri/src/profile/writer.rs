...
/// Write profile.md and profile_meta.json for `scope`. `scope_entries` is the
/// FULL scope-in index (already filtered to this scope's consented sources,
/// no watermark — see `mod.rs::run_refresh`), used to compute the
/// deterministic "Project Activity" table appended as the last section.
pub fn write_profile(
    archive_dir: &Path,
    scope: ProfileScope,
    sections: &ProfileSections,
    meta: &ProfileMeta,
    scope_entries: &[&IndexEntry],
) -> Result<PathBuf, ProfileError> {
    let dir = scope_dir(archive_dir, scope);
    fs::create_dir_all(&dir)?;

    let date = meta.generated_at.get(..10).unwrap_or(&meta.generated_at);
    let markdown = build_profile_markdown(scope, date, meta.session_count, sections, scope_entries);
...
fn build_profile_markdown(
    ...
    // Phase 2 (deterministic, never touched by the LLM): appended LAST so it
    // always trails the model-maintained sections.
    let activity = stats::project_activity_markdown(scope_entries, date);
