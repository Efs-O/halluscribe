...
pub(crate) fn build_chat_system_prompt(context: &ChatPromptContext) -> String {
    ...
    let profile_section = profile_section(&context.profile, context.search_mode);
    format!(
...
pub(crate) fn profile_section(profile: &Option<String>, search_mode: SearchModePrompt) -> String {
    let trimmed = profile.as_deref().map(str::trim).unwrap_or("");
    if trimmed.is_empty() {
        return "User profile:\nNo distilled user profile is available.".to_string();
    }
